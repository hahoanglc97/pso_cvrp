...
public class GraphUI {
	...
	public void displayGraph(String gName, List<Integer> edges){
		...
		String styleSheet = "node { fill-color: blue; text-color: white; size: 40px; text-size: 20px;} " +
	...
	private void explore(Graph graph){
		...
		for(; i< graph.getEdgeCount(); i++){
			graph.getEdge(i).setAttribute("ui.class", "marked");
			if(graph.getEdge(i).getNode1().getId().equals("0"))
				graph.getEdge(i).setAttribute("ui.label", "Trip:"+t++);
			sleep();
		}
		graph.getEdge(i-1).setAttribute("ui.label", graph.getId());
	}
